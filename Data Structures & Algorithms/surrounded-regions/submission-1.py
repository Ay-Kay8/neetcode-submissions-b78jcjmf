...
class Solution:
    def solve(self, board: List[List[str]]) -> None:
        ...
        # O(n * m)
        # There's a better way to do this, only scan the border
                ...
                if board[i][j] == 'O':
                    for dr, dc in dir:
                        ...
                        if new_r not in range(ROWS) or new_c not in range(COLS):
                            # as soon as one of the directions are out of bounds, mark it as safe and
                            # break because we don't need to check other directions
                            board[i][j] = "S"
